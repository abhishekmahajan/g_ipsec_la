...
#define ASFCTRL_DUMMY_SKB_CB_OFFSET	(64)
...
static inline int asfctrl_skb_is_dummy(struct sk_buff *skb)
{
	...
	if (((unsigned char)(skb->cb[ASFCTRL_DUMMY_SKB_CB_OFFSET]) == ASFCTRL_DUMMY_SKB_MAGIC1)
	&& ((unsigned char)(skb->cb[ASFCTRL_DUMMY_SKB_CB_OFFSET+1]) == ASFCTRL_DUMMY_SKB_MAGIC2))
		return 1;
...
/* Subha: 11/10 Changed the prototype to include net */
extern void asfctrl_invalidate_sessions(void);
extern ASF_void_t asfctrl_invalidate_sessions_by_ns(struct net *net);

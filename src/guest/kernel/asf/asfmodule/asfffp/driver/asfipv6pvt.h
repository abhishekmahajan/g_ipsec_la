				...
				ULONG initval)
{
	...
	ulPorts += initval;
	asf_debug("3: ulPorts = 0x%x\n", (unsigned int)ulPorts);
	ASF_BJ3_MIX(ulSrcIp, ulDestIp, ulPorts);
	asf_debug("4: ulSrcIp = 0x%x, ulDestIp = 0x%x, ulPorts = 0x%x\n", (unsigned int)ulSrcIp, (unsigned int)ulDestIp, (unsigned int)ulPorts);
#if (ASF_FEATURE_OPTION > ASF_MINIMUM)
	ulSrcIp += ulVsgId;
	ulDestIp += ulZoneId;
	asf_debug("5: ulSrcIp = 0x%x, ulDestIp = 0x%x, ulPorts = 0x%x\n", (unsigned int)ulSrcIp, (unsigned int)ulDestIp, (unsigned int)ulPorts);
	ASF_BJ3_MIX(ulSrcIp, ulDestIp, ulPorts);
	asf_debug("6: ulSrcIp = 0x%x, ulDestIp = 0x%x, ulPorts = 0x%x\n", (unsigned int)ulSrcIp, (unsigned int)ulDestIp, (unsigned int)ulPorts);
#endif /* (ASF_FEATURE_OPTION > ASF_MINIMUM) */
	asf_debug("ipv6_rule_salt = 0x%x, ulPorts = 0x%x, Hashvalue = 0x%x\n", ipv6_rule_salt, (unsigned int)ulPorts, (unsigned int)(ipv6_rule_salt + ulPorts));
...
static inline int asf_route6_resolve(ASFNetDevEntry_t *inputDev, ffp_flow_t *flow, ASFBuffer_t *abuf)
{
	int ret;
	if ((ret = _asf_route6_resolve(inputDev, &flow->ipv6, abuf)) == 0)
	{
		flow->odev = flow->ipv6.dst->dev;
	}
	return ret;
}
static inline int asf_arp6_resolve(ffp_flow_t *flow, ASFBuffer_t *abuf)
{
	int ret;
	if ((ret = _asf_arp6_resolve(&flow->ipv6, abuf, &flow->ipv6DestIp)) == 0)
	{

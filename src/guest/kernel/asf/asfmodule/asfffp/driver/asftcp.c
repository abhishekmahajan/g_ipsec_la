					...
					asf_vsg_info_t *vsgInfo)
{
	...
	*/
	unsigned int ulSeqNum = ntohl(tcph->seq);
	unsigned int ulAckNum = ntohl(tcph->ack_seq);

	asf_debug("Subha: ulSeqNum = 0x%x, ulAckNum = 0x%x, without ntohl ulSeqNum=0x%x, ulAckNum=0x%x\n", ulSeqNum, ulAckNum, tcph->seq, tcph->ack_seq);

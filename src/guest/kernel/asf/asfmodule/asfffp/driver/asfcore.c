...
#include "asftmr.h"
#include "asfreasm.h"
#if 1 /* Subha 02/11 Part of ARP/ROUTE changes */
#include "asfroute.h"
#include "asfroute6.h"
#endif
#include "asfpvt.h"
...
ASFFwdFn_f pASFFWDFn;
EXPORT_SYMBOL(pASFFWDFn);

ASFFwd6Fn_f pASFFWD6Fn;
EXPORT_SYMBOL(pASFFWD6Fn);


void ASFFFPRegisterFwdFunctions(ASFFwdFn_f pFn)
{
        printk("RegisterFwdFunction called\n");
	pASFFWDFn = pFn;
}
EXPORT_SYMBOL(ASFFFPRegisterFwdFunctions);

void ASFFFPRegisterFwd6Functions(ASFFwd6Fn_f pFn)
{
        printk("RegisterFwdFunction called\n");
	pASFFWD6Fn = pFn;
}
EXPORT_SYMBOL(ASFFFPRegisterFwd6Functions);
...
/* Copied from linux: ip_input.c Subha: Need to check */
...
int asf_process_ip_options(struct sk_buff *skb, struct net_device *dev, struct iphdr *iph)
{
	...
	/* Subha: Patching currently */
	printk("Not handling packets with options: Need to revisit: Subha 01/29\n");
							...
							unsigned int  fqid)
{
	...
	abuf.pAnnot = phys_to_virt(qm_fd_addr(fd));
...
static __sum16 checksum_udp(struct sk_buff *skb, struct udphdr *uh,
			    unsigned short ulen, struct iphdr *iph)
{
	__sum16 csum;
	u32 sum = 0;
	u_int32_t i;
	u_int16_t *buf = (u_int16_t *)((char *)(skb->data + (iph->ihl*4))); /* Start of UDP header */
	...
	/* remove checksum value */
	sum -= htons(buf[3]);


	buf = (u_int16_t *)(&(iph->saddr));

	for (i=0; i < 2; i++)
		sum+= htons(buf[i]);

	buf = (u_int16_t *)(&(iph->daddr));

	for (i=0; i < 2; i++)
		sum+= htons(buf[i]);


	sum += IPPROTO_UDP + ulen;
...
static inline __sum16 calc_udp_csum(struct sk_buff *skb, u_int32_t x_hh_len)
{
	u_int16_t csum_val;
	struct udphdr *uh;
	struct iphdr *iph;
	...
	iph = (struct iphdr *)((char *)(skb->data) + x_hh_len);

	uh = (struct udphdr *)(((char *)iph) + iph->ihl *4);

	ulen = ntohs(uh->len);

	return (checksum_udp(skb, uh, ulen, iph));
}

static int asf_ffp_devfp_rx_int(struct sk_buff *skb, struct net_device *real_dev);
int asf_ffp_devfp_rx_veth(struct sk_buff *skb, struct net_device *real_dev)
{
	if (0 == asf_enable)
		return AS_FP_PROCEED;
	return(asf_ffp_devfp_rx_int(skb, real_dev));
}
EXPORT_SYMBOL(asf_ffp_devfp_rx_veth);
...
int asf_ffp_devfp_rx_int(struct sk_buff *skb, struct net_device *real_dev)
{
#if (ASF_FEATURE_OPTION > ASF_MINIMUM)
	ASFFFPGlobalStats_t	*gstats;
	int			bCsumVerify = 0;
	...
	if (unlikely(skb->pkt_type != PACKET_HOST)) {
		...
		printk("ret_pkt: 1devfp_rx called\n");
		goto ret_pkt;
	}
	...
	if ((usEthType != __constant_htons(ETH_P_IP))
		...
		XGSTATS_INC(NonIpPkts);
		asf_debug_l2("Non IP traffic. EthType = 0x%x\n", usEthType);
		asf_debug("Subha: Non IP traffic. EthType = 0x%x\n", usEthType);
		goto ret_pkt;
	}
...
#if 0 /* Temp  Subha: 09/22 V6 forwarding */
			asf_debug("Subha: calling ASFFFPIPv6ProcessAndSendPkt\n");
			ret = ASFFFPIPv6ProcessAndSendPkt(anDev->ulVSGId,
					anDev->ulCommonInterfaceId,
					abuf, (genericFreeFn_t)ASF_SKB_FREE_FUNC, skb, NULL);
#else
			printk("Caling pASFFWD6Fn: pASFFWD6Fn=%p\n", pASFFWD6Fn);
		        if (pASFFWD6Fn)
			{
				printk("Entering FWD6Fn\n");
	               		ret = pASFFWD6Fn(
					anDev->ulVSGId, 
					anDev->ulCommonInterfaceId,
					abuf,
...
*/
	...
	if (unlikely(skb->ip_summed != CHECKSUM_UNNECESSARY)) {
		...
		if (unlikely(bCsumVerify || (iph->ihl > 5)
			...
			if (likely((iph->protocol == IPPROTO_UDP)
				&& ((udp_csum_val = *(unsigned short *) (skb->data
					+ x_hh_len + iph->ihl*4 + 6)) == 0))) {
				XGSTATS_INC(UdpBlankCsum);
			}
#if 0
			 else {
#if 1 /* for Intel e1000 */
				if (likely(iph->protocol == IPPROTO_UDP))
				{
					csum = calc_udp_csum(skb, x_hh_len);
					if (csum != ntohs(udp_csum_val))
					{
#endif
	...
	if (anDev->ndev)
		skb->dev = anDev->ndev;
	skb->len = ntohs(iph->tot_len); /* Subha 02/04: Added ntohs */
			...
			ASFBuffer_t abuf)
{
	...
	hexdump(abuf.iph, data_len - abuf.pAnnot->parse_result.ip_off[0]);
#endif
	...
	flow = asf_ffp_flow_lookup_DPAA(iph->saddr, iph->daddr,
					*ptrhdrOffset/* ports*/, anDev->ulVSGId,
					...
					ulHashVal);
	asf_debug("ASF: %s Hash(%d.%d.%d.%d, %d.%d.%d.%d, 0x%x, %d, %d)"\
...
drop_pkt:
	...
	if (unlikely(abuf.frag_list))
		...
		dpa_fd_release(abuf.ndev, (struct qm_fd *)abuf.pAnnot->fd);
				...
				)
{
...
#if 1 /* Subha 02/11 Part of ARP_ROUTE_CACHE_IN_FLOW */ 
	int ret;
#endif
	...
	asf_debug("ASF: %s Hash(%d.%d.%d.%d, %d.%d.%d.%d, 0x%lx, %d, %d)"\
		...
		skb->dev->name,
		NIPQUAD(iph->saddr), NIPQUAD(iph->daddr), (u_int32_t)*ptrhdrOffset,
...
// -for testing
		...
		if (vsgInfo) {
			...
			if ((!flow->bIPsecOut) &&
...
#if 0 /* Subha: 02/11 : ROUTE_ARP_CACHE_IN_FLOW */
				...
				L2blobRefresh = ASF_L2BLOB_REFRESH_NORMAL;
#else
				/* Need to release the route and refresh route/arp */
				asf_debug("L2 blob magic number has changed, time to resolve ARP/Route\n");
				/* AAA */

				if (flow->ipv4.n)
				{
					neigh_release(flow->ipv4.n);
					flow->ipv4.n = NULL;
					asf_debug("Setting  flow->ipv4.n = NULL\n");
				}

				if (flow->ipv4.rth)
				{
					dst_release(&flow->ipv4.rth->dst);
					flow->ipv4.rth = NULL;
					asf_debug("Setting  flow->ipv4.rth = NULL\n");
				}

				if (flow->ipv4.res.fi)
				{
					fib_info_put(flow->ipv4.res.fi);
					flow->ipv4.res.fi = NULL;
					asf_debug("Setting  flow->ipv4.res.fi = NULL\n");
				}

				flow->l2blob_len = 0;
				flow->odev = NULL;

				memset(&flow->ipv4.res, 0, sizeof(struct fib_result));
				flow->ipv4.fl.flowi4_iif = NULL;

				ret = asf_ffp_route_resolve(anDev, flow, &Buffer, iph->tos);
		...
		if (iph->protocol == IPPROTO_UDP) {
			...
			XGSTATS_INC(TcpPkts);
			trhlen = ntohl((unsigned short)((*(ptrhdrOffset + 3)) &
							0xf0000000) >> 28) * 4;
...
/* Only timer based L2 blob refresh  is supported in current release */
		...
		if (!flow->bIPsecOut &&
			(flow->l2blob_len == 0)) {
#if 0 /* ROUTE_ARP_CACHE_IN_FLOW 02/11 */
			asf_debug("Generating L2blob Indication as L2blob Not found!\n");
			L2blobRefresh = ASF_L2BLOB_REFRESH_RET_PKT_STK;
			goto gen_indications;
#else
			/* Call fib lookup and cache the route */
		...
		if (flow->bNat) {
			XGSTATS_INC(NatPkts);
			asf_debug_l2("applying NAT\n");
			asf_debug("subha:applying NAT: flow->ulSrcNATIp =0x%x\n", flow->ulSrcNATIp);
...
#if 1 /* Route ARP Changes: Resolve the ARP */
			
			
		if ((flow->ipv4.n) &&  ((flow->ipv4.n->flags & NUD_FAILED) || (flow->ipv4.n->flags & NUD_STALE)))
		{
			/* BBB */
			neigh_release(flow->ipv4.n);
			flow->ipv4.n = NULL;
			...
			asf_debug("Subha: skb->len =0x%x, flow->l2blob_len = %d, flow->pmtu = %d\n", skb->len, flow->l2blob_len, flow->pmtu);

			/* flow->l2blob_len > 0 && flow->odev != NULL
			from this point onwards */
			if (((skb->len + ((flow->l2blob_len > 0) ?
				flow->l2blob_len : LL_RESERVED_SPACE(flow->ipv4.rth->dst.dev))) >
				(flow->pmtu + ETH_HLEN)) ||
				...
				if (!asfIpv4Fragment(skb, flow->pmtu,
					...
					for (; pSkb != NULL; pSkb = pTempSkb) {
						...
						if (!(flow->ipv4.n))
						{
							...
							ret = asf_ffp_arp_resolve(flow, &Buffer);
							asf_debug("asf_ffp_arp_resolve returned %d ret\n", ret);
							if (ret == 2)
							{
...
#if 0 /* Part of ROUTE_ARP_CACHE_IN_FLOW */
			asf_debug_l2("copy l2blob to packet (blob_len %d)\n",
				flow->l2blob_len);
			asfCopyWords((unsigned int *)skb->data,
				(unsigned int *)flow->l2blob, flow->l2blob_len);
#else
			asfCopyWords((unsigned int *)skb->data, (unsigned int *)flow->ipv4.n->ha,  6);
			asfCopyWords((unsigned int *)(skb->data+6), (unsigned int *)flow->ipv4.n->dev->perm_addr, 6);
#endif
#ifdef ASF_IPV6_FP_SUPPORT
			if (flow->bIP4IP6Out) {
				struct ipv6hdr *ipv6h = (struct ipv6hdr *)skb_network_header(skb);
...
asf_vsg_info_t *asf_ffp_get_vsg_info_node(ASF_uint32_t ulVSGId)
{
	...
	if (asf_vsg_info[ulVSGId])
	{
		printk("VSG already created %d: %p\n", ulVSGId, asf_vsg_info[ulVSGId]);
		rcu_read_unlock();
	...
	vsg->bIPsec = 0;
	ASF_SPIN_LOCK(bLockFlag, &asf_vsg_lock);
	asf_vsg_info[ulVSGId] = vsg;
	ASF_SPIN_UNLOCK(bLockFlag, &asf_vsg_lock);
...
void asf_ffp_release_vsg_info_node(ASF_uint32_t ulVSGId)
{
	asf_vsg_info_t *info;
        bool bLockFlag;
	...
	if (asf_vsg_info[ulVSGId])
	{
		info = asf_vsg_info[ulVSGId];
		rcu_read_unlock();
		ASF_SPIN_LOCK(bLockFlag, &asf_vsg_lock);
		asf_vsg_info[ulVSGId] = NULL;
		ASF_SPIN_UNLOCK(bLockFlag, &asf_vsg_lock);
		printk("asf_ffp_release_vsg_info_node ulVSGId=%d found\n", ulVSGId);
	        call_rcu(info,  kfree);
		return;
	}
	else
	{
		printk("asf_ffp_release_vsg_info_node ulVSGId=%dnot found\n", ulVSGId);
		rcu_read_unlock();
		return;
	}
}
...
ASF_uint32_t ASFCreateVSG(ASF_uint32_t ulVSGId, ASF_uint32_t configIdentity)
{
	asf_vsg_info_t *info;
	info = asf_ffp_get_vsg_info_node(ulVSGId);
	if (info != NULL)
	{
		info->configIdentity.ulConfigMagicNumber=configIdentity;
		printk("ASFCreateVSG: VSG created%d\n", ulVSGId);
		return ASF_SUCCESS;
	}
	printk("ASFCreate VSG failed%d\n", ulVSGId);
	return ASF_FAILURE;
}
EXPORT_SYMBOL(ASFCreateVSG);

ASF_uint32_t ASFDeleteVSG(ASF_uint32_t ulVSGId)
{
	asf_ffp_release_vsg_info_node(ulVSGId);
	return ASF_SUCCESS;
}
EXPORT_SYMBOL(ASFDeleteVSG);
#if (ASF_FEATURE_OPTION > ASF_MINIMUM)
...
void endian_check(void)
{
	...
	if (*(short *)test == 1)
		endian_var = 1; /* Little endian */
	else
		endian_var = 2; /* Big Endian */
}
...
static inline int ffp_flow_copy_info(ASFFFPFlowInfo_t *pInfo, ffp_flow_t *flow)
{
	...
	asf_debug("Subha: usSrcPort = 0x%x, usDestPort = 0x%x\n", pInfo->tuple.usSrcPort, pInfo->tuple.usDestPort);
	if (endian_var == 1) /* Little Endian */
		flow->ulPorts = (pInfo->tuple.usDestPort << 16)|pInfo->tuple.usSrcPort;
	else
...
/* Subha: Trying out for Intel Litte Endian */
		...
		asf_debug("Subha: flow creation: flow->tcpState.ulHighSeqNum = 0x%x, ulSeqDelta = 0x%x\n",
			flow->tcpState.ulHighSeqNum, flow->tcpState.ulSeqDelta);
				...
				ffp_flow_t **pFlow1, ffp_flow_t **pFlow2, ULONG *pHashVal)
{
	...
	if (flow1 && flow2) {
...
#if 1 /* Subha: 02/11 ROUTE_ARP_CACHE_IN_FLOW */
		fl1 = &flow1->ipv4.fl;
		fl2 = &flow2->ipv4.fl;

		fl1->flowi4_oif = 0;
		fl1->flowi4_scope = RT_SCOPE_UNIVERSE;
		fl1->daddr = flow1->ulDestIp;
		fl1->saddr = flow1->ulSrcIp;

		fl2->flowi4_oif = 0;
		fl2->flowi4_scope = RT_SCOPE_UNIVERSE;
		fl2->daddr = flow2->ulDestIp;
		fl2->saddr = flow2->ulSrcIp;
#endif
...
#ifdef ASF_IPV6_FP_SUPPORT
		if (bIPv6_flow1 == true) {
		asf_debug("ComputeFlowHash: (%x:%x:%x:%x:%x:%x:%x:%x, %x:%x:%x:%x:%x:%x:%x:%x, 0x%lx, %d, %d)\n",
			PRINT_IPV6_OTH(flow1->ipv6SrcIp), PRINT_IPV6_OTH(flow1->ipv6DestIp), flow1->ulPorts, ulVsgId, flow1->ulZoneId);
		...
		if (bIPv6_flow2 == true) {
		asf_debug("ComputeFlowHash: (%x:%x:%x:%x:%x:%x:%x:%x, %x:%x:%x:%x:%x:%x:%x:%x, 0x%lx, %d, %d)\n",
			PRINT_IPV6_OTH(flow2->ipv6SrcIp), PRINT_IPV6_OTH(flow2->ipv6DestIp), flow2->ulPorts, ulVsgId, flow2->ulZoneId);
...
#if (ASF_FEATURE_OPTION > ASF_MINIMUM)
		if (!flow1->bIPsecOut) {
#if 0 /* Part of ARP/Routing changes Subha */
		...
		asf_inac_divisor = 1;
	//	printk("inactivity timer = %d InacTime = %d, asf_inac_divisor =%d\n", flow1->ulInacTime/asf_inac_divisor, flow1->ulInacTime, asf_inac_divisor);
...
static int ffp_cmd_delete_flows(ASF_uint32_t  ulVsgId, ASFFFPDeleteFlowsInfo_t *p, ULONG *pHashVal)
{
	...
	asf_debug("Subha: Flow to be deleted : bIPv6 = %d\n", bIPv6);
	...
	if (bIPv6 == true)
		...
		flow1 = asf_ffp_flow_lookup_in_bkt_ex(&p->tuple, ulVsgId, p->ulZoneId, (ffp_flow_t *)  bkt1);
			asf_debug("Subha: flow1 = 0x%x: \n", flow1);
	if (flow1) {
		if (unlikely(flow1->bDeleted)) {
			spin_unlock_bh(&bkt1->lock);
			asf_debug("Subha: flow1->bDeleted = %d: returning\n", flow1->bDeleted);
			return -1;
		}
		...
		if (flow2) {
			...
			if (rem_flow2_resources) {
#if 0 /* Subha: Part of Route ARP Cache in Flow */
				if (flow2->pL2blobTmr) {
					asfTimerStop(ASF_FFP_BLOB_TMR_ID, 0, flow2->pL2blobTmr);
				}
#endif
				...
				if (bIPv6 == true)
				{
					if (flow2->ipv6.dst)
					{
						dst_release(flow2->ipv6.dst);
						flow2->ipv6.dst = NULL;
					}

					if (flow2->ipv6.n)
					{
						neigh_release(flow2->ipv6.n);
						flow2->ipv6.n = NULL;
					}
				}
				else
#endif
				{
					...
					{
						fib_info_put(flow2->ipv4.res.fi);
						flow2->ipv4.res.fi = NULL;
					}

					if (flow2->ipv4.rth)
					{
						dst_release(&(flow2->ipv4.rth->dst));
						flow2->ipv4.rth = NULL;
					}

					if (flow2->ipv4.n)
					{
						neigh_release(flow2->ipv4.n);
						flow2->ipv4.n = NULL;
					}
				}
		...
		if (bIPv6 == true)
		{
			if (flow1->ipv6.dst)
			{
				dst_release(flow1->ipv6.dst);
				flow1->ipv6.dst = NULL;
			}

			if (flow1->ipv6.n)
			{
				neigh_release(flow1->ipv6.n);
				flow1->ipv6.n = NULL;
			}
...
#endif
		{
			...
			{
				fib_info_put(flow1->ipv4.res.fi);
				flow1->ipv4.res.fi = NULL;
			}


			if (flow1->ipv4.rth)
			{
				dst_release(&(flow1->ipv4.rth->dst));
				flow1->ipv4.rth = NULL;
			}
			     ...
			     unsigned int ulIndex, unsigned int ulMagicNum, unsigned int ulHashVal, bool bIPv6)
{
	...
	if (asf_enable) {
		...
		/* Subha: 01/30: Changed from %lu to %u */
		asf_debug("Blob Tmr: flow not found {%u, %u}.. (might happen while flows are being deleted)!!!\n",
			  ulIndex, ulMagicNum);
	}
...
static int __init asf_init(void)
{
	...
	endian_check();
	asf_debug("Endianness = %d: %s\n", endian_var, ((endian_var == 1) ? "Little Endian" : "Big Endian"));
#ifdef ASF_TOOLKIT_SUPPORT
...
#if 1 /* Subha: Routing/ARP Changes */
	asf_ipv4_dst_blackhole_ops.kmem_cachep = kmem_cache_create("asf_ip_dst_cache", sizeof(struct rtable), 0,
		SLAB_HWCACHE_ALIGN|SLAB_PANIC, NULL); 
	if (asf_ipv4_dst_blackhole_ops.kmem_cachep == NULL)
	{
		asf_debug("KERN INFO dst_cache initialization failed\n");
		return err;
	}
	printk("Subha: asf_ipv4_dst_blackhole_ops.kmem_cachep = 0x%p\n", asf_ipv4_dst_blackhole_ops.kmem_cachep);
	if (dst_entries_init(&asf_ipv4_dst_blackhole_ops) < 0)
	{
		asf_debug("KERN INFO dst_entries_init failed\n");
		return err;
	}
	printk("Subha: asf_ipv4_dst_blackhole_ops.kmem_cachep = 0x%p\n", asf_ipv4_dst_blackhole_ops.kmem_cachep);
#endif
...
#if 1 /* Subha: Route/ARP Changes */
	dst_entries_destroy(&asf_ipv4_dst_blackhole_ops);
	kmem_cache_destroy(asf_ipv4_dst_blackhole_ops.kmem_cachep);
#endif

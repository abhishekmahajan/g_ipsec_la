 ...
 *
 * Revisions:   
 *       0.1   Route/ARP Lookup instead of L2blob <b22166@freescale.com>
*/
/******************************************************************************/
...
#include "asfmpool.h"
#include "asftmr.h"
#if 1 /* Part of ARP/Route Changes: Subha 03/17 */
#include "asfroute.h"
#include "asfroute6.h"
#endif
#include "asfreasm.h"
				...
				)
{
	...
	asf_debug("Subha: ProcessAndSendIPv6: 1\n");
	...
	asf_debug("Subha: ProcessAndSendIPv6: 5\n");

	if (unlikely((skb->len < pkt_len))) {
#if (ASF_FEATURE_OPTION > ASF_MINIMUM)
		gstats->ulErrIpHdr++;
#endif
		asf_debug("Subha: skb_len = 0x%x, pkt_len = 0x%x\n", skb->len, pkt_len);
		goto drop_pkt;
	}
	asf_debug("Subha: ProcessAndSendIPv6: 6\n");
	...
	asf_debug("Subha: ProcessAndSendIPv6: 7\n");
#endif
	asf_debug("nexthdr = 0x%x, NEXTHDR_ESP = 0x%x, NEXTHDR_AUTH =0x%x\n", nexthdr, NEXTHDR_ESP, NEXTHDR_AUTH);
#ifdef ASF_IPSEC_FP_SUPPORT
	...
	if (unlikely((nexthdr != NEXTHDR_TCP) &&
		(nexthdr != NEXTHDR_UDP))) {
			...
			if (pIpsecOpaque && pFFPIpsecInVerify) {
				...
				asf_debug("Subha: 2 ASF_DONE\n");
				return ASF_DONE;
			}
#endif
		/* Dont process non TCP/UDP packets */
		/* return packet to linux stack */
		asf_debug("Subha: Returning packet to stack: Not TCP or UDP\n");
		return ASF_RTS;
	}

	asf_debug("Subha: ProcessAndSendIPv6: 8\n");

	//ptrhdrOffset = (unsigned int *)((unsigned char *) skb_transport_header(skb));
	ptrhdrOffset  = (unsigned int *)((unsigned char *)ip6h + sizeof(struct ipv6hdr));
	...
	if (vsgInfo) {
		...
		if ((!flow->bIPsecOut) &&
...
#if 0 /* Part of ARP/Route Changes */
			if (!flow->configIdentity.l2blobConfig.bl2blobRefreshSent) {
				flow->configIdentity.l2blobConfig.ulOldL2blobJiffies = jiffies;
				flow->configIdentity.l2blobConfig.bl2blobRefreshSent = 1;
			}
			...
			L2blobRefresh = ASF_L2BLOB_REFRESH_NORMAL;
#else
			asf_debug("L2blob changed: Time to resolve Route & ARP \n");
	...
	if ((flow->ipv6.n) &&  ((flow->ipv6.n->flags & NUD_FAILED) || (flow->ipv6.n->flags & NUD_STALE)))
	{
		/* BBB */
		neigh_release(flow->ipv6.n);
		flow->ipv6.n = NULL;
	...
	asf_debug("Subha: ProcessAndSendIPv6: 9\n");
	if (!flow->bIPsecOut &&
		(flow->l2blob_len == 0)) {
#if 0 /* Part of ARP/Toute Changes Subha 03/17 */
		asf_debug("Generating L2blob Indication as L2blob Not found!\n");
		L2blobRefresh = ASF_L2BLOB_REFRESH_RET_PKT_STK;
		goto gen_indications;
#else
		/* do route lookup */
		if (unlikely(!flow->ipv6.dst))
		{
			if (unlikely((ret = asf_route6_resolve(anDev, flow, &Buffer)) != 0))
			{
				asf_debug("Unable to resolve route: ret = %d Send Packet up to stack\n",ret);
				/* return skb to Linux stack */
				return ASF_RTS;
			}
			asf_debug("asf_route6_resolve returned %d\n", ret);
		...
		/* do arp lookup */
		if ((ret = asf_arp6_resolve(flow, &Buffer)) == 0)
		{
			asf_debug("arp6_resolve returned %d\n", ret);
...
#endif
			XGSTATS_INC(TcpHdrLenErr);
			asf_debug("Subha: Packet dropped due to invalid length check\n");
			asf_debug("Subha: skb->len = 0x%d, exthdrsize = %d, sizeof(struct ipv6hdr) = %d, trhlen = %d\n",
				skb->len, exthdrsize, sizeof(struct ipv6hdr), trhlen);
			asfFfpSendLog(flow, ASF_LOG_ID_INVALID_TCP_HDRLEN, ulHashVal);
			goto drop_pkt;
		}
	asf_debug("Subha: ProcessAndSendIPv6: 10\n");
#if (ASF_FEATURE_OPTION > ASF_MINIMUM)
		ptcph = (struct tcphdr *) ptrhdrOffset;
		/*optlen = ptcph->doff*4-20; */
		...
		asf_debug("Subha: ProcessAndSendIPv6: 11\n");
		oth_flow = ffp_ipv6_flow_by_id(&flow->other_id);
		if (!oth_flow) {
			asf_debug("other flow is not found!! strange!!\n");
			goto drop_pkt;
...
#endif /* (ASF_FEATURE_OPTION > ASF_MINIMUM) */
		asf_debug_l2("TCP state processing is done!\n");
		asf_debug("Subha: ProcessAndSendIPv6: 12\n");
	}
#if (ASF_FEATURE_OPTION > ASF_MINIMUM)
	flow_stats->ulInPkts++;

#if 0 /* Part of ARP/Routing changes Subha 03/17 */
/* Only timer based L2 blob refresh  is supported in current release */
	if (asf_l2blob_refresh_npkts &&
		(flow_stats->ulInPkts % asf_l2blob_refresh_npkts) == 0) {
		asf_debug_l2("Decided to send L2Blob refresh ind based on npkts\n");
	...
	if (ASF_QM_NULL_DSCP != flow->mkinfo.uciDscp)
		hdr->tc = flow->mkinfo.uciDscp;
#endif
	asf_debug("Subha: ProcessAndSendIPv6: 13\n");
	...
	asf_debug("Subha: ProcessAndSendIPv6: 14\n");
...
#if 0 /* Part of ARP Route Changes: Subha 03/17 */
		asfCopyWords((unsigned int *)skb->data, (unsigned int *)flow->l2blob, flow->l2blob_len);
#else
		asf_debug("Calling asfCopyWords\n");
		asfCopyWords((unsigned int *)skb->data, (unsigned int *)flow->ipv6.n->ha, 6 );
		asfCopyWords((unsigned int *)(skb->data + 6), (unsigned int *)flow->ipv6.n->dev->perm_addr,  6);
#endif
		if (flow->bIP6IP4Out) {
